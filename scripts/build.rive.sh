...
copy_runtime_headers() {
    ...
    # The Objective-C++ host-module adapter compiles against these private
    # runtime dependencies. They are not exported as public framework headers.
    local deps_dir
    ...
    if [ -n "$miniaudio_header" ]; then
        ...
        cp "$miniaudio_header" $DEV_SCRIPT_DIR/../dependencies/includes/miniaudio/miniaudio.h
    fi
}
